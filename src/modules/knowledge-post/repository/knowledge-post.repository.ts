...
class KnowledgePostRepository implements ICursorPagination<
  Prisma.KnowledgePostWhereInput,
  any
> {
  findAll({
    after,
    ...
    after?: string;
    take: number;
    where?: Prisma.KnowledgePostWhereInput | undefined;
    props?: any;
  }): Promise<any[]> {
  ...
  async create(
    payload: CreateKnowledgePostPayload,
  ): Promise<KnowledgePostRecord> {
  ...
  async findById(id: string): Promise<any> {
    return prisma.knowledgePost.findUnique({
      where: { id },
      include: {
        user: {
          select: {
